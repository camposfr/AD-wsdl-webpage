...
?>
      ...
      <div class="content-wrapper">
        ...
        <section class="content-header">
          <h1>
            ADGroups Automation User Creation				
          </h1>
          <ol class="breadcrumb">
...
if ($conn){
    if(($result = sqlsrv_query($conn,"SELECT * FROM ADGroupUsers")) !== false){
		$x = 0;
        while( $obj = sqlsrv_fetch_object( $result )) {
					   ...
					   echo '<td>',$obj->id,'</td>';
                       echo '<td>',$obj->usr,'</td>';
                       echo '<td>',$obj->usr_role,'</td>';
					   echo '<td><a href="http://<server>/admin/createUser.php?operation=deleteusr&ID=',$x,'"><button type="submit" class="btn btn-primary btn-danger" id="',$x,'">ERASE</button></a></td>';
                       echo '</tr>';
        }
    }
}else{
                ...
                <form role="form">
                  <div class="box-body">
                    <div class="form-group">
                      <label for="Application1">User</label>
                      <input type="text" class="form-control" id="User1" placeholder="Must be at least 4 chars and up to 10 chars, No special characters">
                    </div>
                    <div class="form-group">
                      <label for="HPSMRequestType1">Password</label>
                      <input type="password" class="form-control" id="Password1" placeholder="Must be at least 6 chars and up to 10 chars, No special characters">
                    </div>
                    <div class="form-group">
                      <label for="TicketType1">Confirm Password</label>
                      <input type="password" class="form-control" id="Password2" placeholder="Confirm Password">
                    </div>			
                    <!-- select -->
                    <div class="form-group">
                      <label>Select Role</label>
                      <select class="form-control" id="RoleSel">
                        <option>user</option>
                        <option>admin</option>
                      </select>
                    </div>					
                    <div class="checkbox">
                      <label>
                        <input type="checkbox" id="checksure1"> Are you 100% sure?
                      </label>
                    </div>
                  </div><!-- /.box-body -->

                  <div class="box-footer">
                    <button type="submit" class="btn btn-primary" id="submit1" onclick="addRequest()">Submit</button>
                  </div>
                </form>
              </div><!-- /.box -->
            </div><!-- /.col -->
	...
	function addRequest() 
	{
		...
		var roles = document.getElementById('RoleSel');
		var rolest = roles.options[roles.selectedIndex].text;
		var URI = "../../createUser.php?operation=createusr";
		URI += "&usr=";
		URI += user;
		URI += "&pass=";
		URI += password;
		...
		URI += "&role=";
		URI += rolest;
		if ((document.getElementById('checksure1').checked) && (password == password2) && (password.length >= 6) && (password.length <= 10) && (checkPass() == true) && (user.length >= 4) && (user.length <= 10))  {
			window.location.href = URI;		
        } else {
            alert("You didn't even VERIFY!");
        }
	}
	</script>
