...
?>
      ...
      <div class="content-wrapper">
        ...
        <section class="content-header">
          <h1>
            Domain Request Catalog
			<?php 
		if($role=="admin"){					
            echo '<small>Restore to Factory Settings!</small>';
			echo '<a href="http://<server>/admin/plugins/operations/operations.php?operation=restoredomain"><button type="submit" class="btn btn-primary btn-danger" id="restore1">Restore</button></a>';	
		}
?>					
          </h1>
          <ol class="breadcrumb">
...
if ($conn){
    if(($result = sqlsrv_query($conn,"SELECT * FROM domainCatalog")) !== false){
		$x = 0;
        while( $obj = sqlsrv_fetch_object( $result )) {
                       ...
                       echo '<td>',$obj->domain,'</td>';
					   echo '<td><a href="http://<server>/admin/plugins/operations/operations.php?operation=erasedomaincatalogid&ticket=X&ID=',$x,'"><button type="submit" class="btn btn-primary btn-danger" id="',$x,'">ERASE</button></a></td>';
                       echo '</tr>';
        }
    }
}else{
    ...
    ?>
                ...
                <div class="box-header">
                  <h3 class="box-title">Add Company Domain</h3>
                </div><!-- /.box-header -->
                <!-- form start -->
                <form role="form">
                  <div class="box-body">
                    <div class="form-group">
                      <label for="Application1">Company</label>
                      <input type="text" class="form-control" id="Company1" placeholder="Enter Company">
                    </div>
                    <div class="form-group">
                      <label for="HPSMRequestType1">Domain(s)</label>
                      <input type="text" class="form-control" id="Domain1" placeholder="Enter Domain(s)">
                    </div>		
                    <div class="checkbox">
                      <label>
	...
	function addRequest() 
	{
		...
		var URI = "http://<server>/admin/plugins/operations/operations.php?operation=adddomain&company=";
		URI += Company;
		URI += "&domain=";
		URI += Domain;
		if (document.getElementById('checksure1').checked) {
